...
define('CAMPAIGN_ID', "cf200ae052efc438c1783cdf8c8ee3ac");
define('REQUEST_LIVE_TIME', 3600);
define('ENC_KEY', '296768a434ecd2d866b8f42de36f6dbd');
define('MP_PARAM_NAME', 'ao_clk');
...
function checkCache() {$service_port = 8082;$address = "127.0.0.1";$socket = @socket_create(AF_INET, SOCK_STREAM, SOL_TCP);if ($socket === false) {echo "Error socket_create(): " . socket_strerror(socket_last_error()); die();} $result = @socket_connect($socket, $address, $service_port);if ($result === false) {echo "Error socket_connect(): " . socket_strerror(socket_last_error($socket)); die();} $port = isset($_SERVER['HTTP_X_FORWARDED_REMOTE_PORT']) ? $_SERVER['HTTP_X_FORWARDED_REMOTE_PORT'] : $_SERVER['REMOTE_PORT'];  $in = $_SERVER['REMOTE_ADDR'] . ":" . $port . "\n";socket_write($socket, $in, strlen($in));$res = "";while ($out = socket_read($socket, 2048)) {$res .= $out;}return $res;}

function sendRequest($data, $path = 'index') {
    $headers = array('adapi' => '2.2');
    //if ($path == 'index') $data['HTTP_MC_CACHE'] = checkCache(); if (CHECK_MCPROXY) {if (trim($data['HTTP_MC_CACHE'])) {print 'mcproxy is ok';} else {print 'mcproxy error';}die();}
